//============================================================
//
//  config.c - SDL configuration routines
//
...
static const options_entry config_opts[] =
{
...
#if defined(__WIN32__) || defined(SDLMAME_MACOSX)
	...
	{ "video",                    "soft",     0,                 "video output method: none, soft or opengl" },
	...
	{ "fullstretch;fs",	      "1",    	  OPTION_BOOLEAN,    "allow non-integer scaling" },
	...
	{ "filter;glfilter;flt",      "1",    OPTION_BOOLEAN, "enable bilinear filtering on screen output" },
        { "waitvsync;glwaitvsync",    "0",    OPTION_BOOLEAN, "wait for refresh" },
	...
	{ "resolution0;resolution;r0;r", "auto",  0,                 "preferred resolution of the first screen; format is <width>x<height>[@<refreshrate>]] or 'auto'" },
	...
	{ "beam",                     "1.1",      0,                 "set vector beam width" },
...
static void extract_options(const game_driver *driver, machine_config *drv)
{
	...
	// DLD: attenuation = options_get_int("volume", TRUE);
